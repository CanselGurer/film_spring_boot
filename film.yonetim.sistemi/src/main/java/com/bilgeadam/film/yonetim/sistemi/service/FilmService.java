...
public class FilmService {
	...
	if (filmById.isEmpty()){
		throw new IllegalStateException("Verilen ıd ile film bulunamadı");
	}
		return filmById;
	}
	...
	public void filmGuncelle(Integer id, String ad, LocalDate vizyonTarihi, BigDecimal hasilat) {
		...
		Film film= filmRepository.findById(id).orElseThrow(()-> new IllegalStateException("Verilen ıd ile film bulunamadı"));
		...
		filmRepository.save(film);//Save metotu burda update işlemi gerçekleştirir.
	}

	public void filmGuncelle(Integer id, Film film) {
		Film varolanFilm= filmRepository.findById(id).orElseThrow(()-> new IllegalStateException("Verilen ıd ile film bulunamadı"));
